...
由于libhv.so链接不上，所以在/etc/ld.so.conf.d/下创建
vi rush-rpc.conf 
/usr/local/lib
刷新缓存
sudo /sbin/ldconfig
...
3、集群搭建
搭建K8s集群service集群（server 3pod client 1pod） 

本地生成docker镜像
直接打包可执行文件 dockerfile
添加so文件搜索路径
在/etc/ld.so.conf.d/下 sudo vim rush-rpc(可以改为复制文件)
内容是 /home/ubuntu
刷新
sudo ldconfig
创建dockerfile
sudo docker build -f dockerfile -t thacher_server:1.0.0 . 
生成镜像
发布到腾讯云镜像仓库
docker login ccr.ccs.tencentyun.com --username=100015196349
docker tag [imageId] ccr.ccs.tencentyun.com/rpc-hub/rush-rpc-client:[tag]
docker push ccr.ccs.tencentyun.com/rpc-hub/rush-rpc-client:[tag]
pod中主动拉
docker pull ccr.ccs.tencentyun.com/rpc-hub/rush-rpc-client:[tag]
创建pod

为防止镜像退出
在deployment中添加
command: ["/bin/bash", "-ce", "tail -f /dev/null"]
也可以在dockerfile文件的CMD添加.sh脚本
脚本最后执行tail -f /dev/null
CMD["*.sh"]
spec:
      containers:
      # 运行容器名称；
 ...
 curl -v http://127.0.0.1:8080/add?name=teacher4&grade=7&subject=history&school=school4

 redis操作命令
 redis-cli

现有代码实现下 不同pod间访问不通
k8s中不同pod访问 必须通过service_ip

http://172.16.3.6:8080/add?name=teacher4\&grade=7\&subject=history\&school=school4
...
教师信息管理系统
wrk测通信延时 
http协议下：写http-http-server测试程序

对照日志，Client节点的系统运行流程简单描述如下，在程序启动之初建立与Etcd的长连接，并用连接池来管理，接着进行服务发现，在Etcd数据库中查找key为“echo.EchoService”的键值对，得到了服务节点的ip：101.33.247.88和port：7749，以及负载信息，接着选择CPU较低负载的机器进行服务调用
